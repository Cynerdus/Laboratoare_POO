...
class BankReport {
    ...
    public static int getNumberOfCustomers(Bank bank) {
        ...
        for (Business business : bank.getClients()) {
            numberOfCustomers += business.getEmployees().size();
        }
    ...
    public static SortedSet<Employee> getCustomersSorted(Bank bank) {
        ...
        SortedSet<Employee> sortedSet = new TreeSet<>();

        for (Business business : bank.getClients()) {
            sortedSet.addAll(business.getEmployees());
        }
    ...
    public static SortedSet<Account> getAccountsSortedBySum(Bank bank) {
        ...
        SortedSet<Account> sortedSet = new TreeSet<>();
    ...
    public static Map<Employee, Collection<Account>> getCustomerAccounts(Bank bank) {
        ...
        HashMap<Employee, Collection<Account>> hashMap = new HashMap<>();
        for (Business business : bank.getClients()) {
            for (Employee employee : business.getEmployees()) {
                hashMap.put(employee, employee.getAccounts());
            }
        }
        ...
        for (Business business : bank.getClients()) {
            for (Employee employee : business.getEmployees()) {
                if (hashMap.containsKey(employee.getCity())) {
                    hashMap.get(employee.getCity()).add(employee);
                } else {
                    hashMap.put(employee.getCity(), new ArrayList<>());
                    hashMap.get(employee.getCity()).add(employee);
                }
            }
        }
...
class BusinessReport {
    ...
    public static long getNumberOfDifferentProjectsWorkedByCurrentFemaleEmployees(Business business) {
        ...
        List<Employee> femaleEmployees = business.getEmployees().stream().filter(e -> e.getGender().equals(Gender.FEMALE)).toList();
    ...
    public static Map<Religion, Map<Gender, List<Employee>>> getEmployeesOnSameReligionAndGender(Business business) {
        ...
        Map<Religion, List<Employee>> religionMap =
        ...
        for (Map.Entry<Religion, List<Employee>> entry : religionMap.entrySet()) {
            religionGenderMap.put(entry.getKey(),
...
class Account implements Comparable<Account> {
    ...
    public int compareTo(Account o) {
        return (balance > o.getBalance()) ? 1 : (balance < o.getBalance()) ? -1 : Integer.compare(id, o.getId());
    }
}
